...
Similarly, `config/crowdsec/profiles.yaml` is generated from `profiles-base.yaml`. The CAPTCHA remediation profile is conditionally injected only if the CAPTCHA provider variables are properly configured in `.env`.
