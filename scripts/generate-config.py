...
def generate_configs():
    ...
    if raw_entries:
        parts = []
        if stats['standard'] > 0: parts.append(f"{stats['standard']} standard")
        if stats['anubis'] > 0:   parts.append(f"{stats['anubis']} Anubis protected")
        if stats['apache'] > 0:   parts.append(f"{stats['apache']} Apache legacy")
        print(f"   ✔ Dynamic configuration generated for {len(raw_entries)} domains ({', '.join(parts)}).")
